...
export class WeChatApi {
   ...
   *  Throws if all retries exhausted — caller should handle backoff. */
  async sendMessage(req: SendMessageReq): Promise<void> {
    const MAX_RETRIES = 3;
    let delay = 15_000; // start with 15s backoff on rate-limit
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      const res = await this.request<{ ret?: number; retmsg?: string }>('ilink/bot/sendmessage', req);
      if ((res as any)?.ret === -2) {
        if (attempt === MAX_RETRIES) {
          throw new Error('SEND_RATE_LIMITED');
        }
        logger.warn('sendMessage rate-limited (ret:-2), retrying', { attempt, delayMs: delay });
        await new Promise(r => setTimeout(r, delay));
        delay = Math.min(delay * 2, 120_000); // exponential backoff, cap at 120s
        continue;
      }
      // ret !== -2: success (or other non-retryable error)
      if ((res as any)?.ret !== undefined && (res as any)?.ret !== 0) {
        logger.warn('sendMessage returned non-zero ret', { ret: (res as any)?.ret, retmsg: (res as any)?.retmsg });
      }
      return;
    }
  }
